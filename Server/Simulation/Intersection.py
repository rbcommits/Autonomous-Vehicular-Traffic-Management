...
class Intersection:
    ...
    def restoreVelocities(self, carList):
        ...
        elif self.IntersectionList and values.conventionalSimFlag is False:
            if(values.conventionalSimFlag and self.IntersectionList[i].positionX < self.positionX and values.conventionalStoppedX and self.IntersectionList[i].velocityX > 0):
                    print("X CAR IS STOPPED")
                    self.IntersectionList[i].velocityX = 0
            elif(values.conventionalSimFlag and self.IntersectionList[i].positionY < self.positionY and values.conventionalStoppedY and self.IntersectionList[i].velocityY > 0):
                    print("Y CAR IS STOPPED")
                    self.IntersectionList[i].velocityY = 0

            elif(values.conventionalSimFlag and self.IntersectionList[i].direction is "vertical" and self.IntersectionList[i].positionX < self.positionX and not values.conventionalStoppedX):
                    self.IntersectionList[i].velocityX = values.maxVelocity

            elif(values.conventionalSimFlag and self.IntersectionList[i].direction is "horizontal" and self.IntersectionList[i].positionY < self.positionY and not values.conventionalStoppedY):
                    self.IntersectionList[i].velocityY = values.maxVelocity
